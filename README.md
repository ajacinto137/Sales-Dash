...
currently assigned a **team** (`sales_reps.team`, see "Sales Rep Roles /
Teams" below) — an explicit per-rep Admin action, same reasoning as
every other Bulk Account View staying source-data-only elsewhere: this
one chart needs a curated rep list, not every name that happens to
appear in the raw sales data. This filtering happens in `app.py` before
...
**Reversed 2026-08-19:** this chart briefly (2026-08-18–19) *also*
required the rep to be mapped to a `users` row with role exactly
`"Sales Rep"` (`user_store.list_sales_rep_role_names()`, since removed).
That extra requirement was found, in a real deployment, to hide every
rep on every team — team assignment (Admin Portal → Sales Reps tab)
deliberately needs no `users` row at all (a rep can rack up sales having
never been given a login, see above), so almost no rep ever satisfied
*both* "has a team" and "has a Sales Rep-role login" at once. Team
assignment alone is now the one signal this chart uses.
...
- Two more view keys exist *outside* the registry: `channel` and `date`
  ...
  in `app.py`). The Team Leaderboard's own Sales Calendar gained the same
  drill-down 2026-08-19 (see the second `date`-view bullet below) — both
  now resolve through `get_calendar_day_account_view()`, just via
  different routes (`bulk_account_view()` rep-scoped vs. `all_sales_view()`
  team-wide, `rep=None`).
  ...
  - **Rep Calendar daily-sales drill-down** (`view=date`, reworked
    ...
    to agree.
  - **Team Sales Calendar daily-sales drill-down** (`view=date` on
    `all_sales_view()`/`/dashboard/accounts`, added 2026-08-19): same
    `.td-calendar-count-link` treatment and 0-count-stays-non-clickable
    rule as the Rep Calendar above, now also on the Team Leaderboard's
    own Sales Calendar (`dashboard.html`'s Sales Calendar & Monthly Sales
    Trend section). The one added wrinkle here: that calendar has its own
    Team/NJ/NY/VA **state**-scoped toggle (`cal_view`/`calendar_views`,
    the sale's `state` column — see "Sales Rep Roles / Teams" for why
    this is a *different* Team/NJ/NY/VA from the Sales Volume tab's rep
    **team** selector), so the drill-down carries `cal_view` through as a
    query param and applies the identical state filter before counting,
    via `DASHBOARD_STATE_VIEWS`/`_state_filtered()`/`_state_code_for_view()`
    — now module-level in `app.py` (hoisted out of `dashboard_page()`
    2026-08-19) specifically so `all_sales_view()` reads the *same*
    definition of "NJ" rather than a second one that could quietly drift.
    Without this, clicking a day while viewing the NJ panel could open a
    Bulk Account View listing every state's sales for that day — the
    displayed count and the drill-down's row count would disagree. The
    resulting page's title gets a `— NJ`/`— NY`/`— VA` suffix when scoped
    (`all_sales_view()` in `app.py`) so it's clear which state's accounts
    are shown; unscoped ("Team") gets no suffix, unchanged from before.
...
Channel Report, served live: it runs the full data-cleaning +
channel-attribution pipeline (`marketing_cleaning.py`) against PlanetWeb
through `run_cleaning_cached()`, which caches the result in-memory for
**one hour** (`CACHE_TTL_SECONDS`) rather than re-running it on every
page load. Originally ran the full ~10-second pipeline (against ~15,000
rows) on every single request, by explicit request ("clean on every
